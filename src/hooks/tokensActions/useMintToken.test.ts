...
describe("useMintToken", () => {
  ...
  test("should return mint function and state", () => {
    const { result } = renderHook(() => useMintToken());

    expect(result.current.mint).toBeInstanceOf(Function);
  ...
  test("should return undefined and show error when user is not connected", async () => {
    ...
    const mintResult = await act(async () => {
      return await result.current.mint({ token: mockToken, amount: "10" });
    });

    expect(mintResult).toBeUndefined();
    expect(mockShowError).toHaveBeenCalledWith(
      "Mint failed: Wallet not connected",
    );
  });
  ...
  test("should call writeContractAsync with correct parameters when everything is valid", async () => {
    ...
    const mintResult = await act(async () => {
      return await result.current.mint({ token: mockToken, amount: "1.0" });
    });
    ...
    expect(mockWriteContractAsync).toHaveBeenCalledWith({
      ...
      functionName: "mint",
      args: [mockUserAddress, 1000000000000000000n],
    });
    expect(mintResult).toBe(mockTransactionHash);
  });
  ...
  test("should handle contract execution errors gracefully", async () => {
    ...
    expect(mintResult).toBeUndefined();
    expect(mockShowError).toHaveBeenCalledWith(
      "There was an error minting the token",
    );
  });

  test("should use correct decimals from store for amount parsing", async () => {
    mockUseTokensStore.mockReturnValue({
      tokensDecimals: { DAI: 18, USDC: 6 }, // DAI has 18 decimals
    } as unknown as TokensStore);

    const { result } = renderHook(() => useMintToken());

    await act(async () => {
      await result.current.mint({ token: mockToken, amount: "1.5" });
    });
  ...
  test("should handle token with different decimals correctly", async () => {
    ...
    await act(async () => {
      await result.current.mint({ token: usdcToken, amount: "100.5" });
    });
